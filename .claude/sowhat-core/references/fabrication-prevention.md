...
**산출물 레이어 (외부 공유):**
- `draft` — 최종 산출물 생성 시 기획에 없던 구체값 추가 금지 (L4 이후 경로, 최후의 L1 적용 지점)
...
Guarantees:
  - **4-way 대칭 매칭** (한·영 × 화이트리스트·접미어):
    - en_direct / ko_direct: 기관명 화이트리스트 + 연도 + 수치 (한·영 **동일 조건**, C1 해소 — ko도 연도·수치 요구)
    - en_suffix / ko_suffix: 기관 접미어 패턴 + 연도 + 수치
  - 매칭 발생 시 같은 불릿 안에 출처 표기(URL / file: / dir: / #NNN / DOI / 복합 cross_ref) 형식 존재 필수
  - 출처 표기 없으면 reject
  - 불릿 단위 처리로 무관한 두 불릿 간 cross-match 방지 (M2 해소)
  - 문장 경계(`[^.!?\n]*?`)로 같은 불릿 내 두 문장 간 cross-match 방지 (M2 해소)
  - Exception은 **복합 조건**:
    · self_data 키워드 단독으로 부족 — `(n=\d+)` / `표본 \d+명` / `file:`/`dir:` 동반 필수
    · `§` 단독 부족 — `§\d+` 숫자 식별자 필수
    · `Appendix` 단독 부족 — `Appendix [A-Z\d]+` 식별자 필수
    (C2 해소 — 키워드 prefix/suffix만으로 bypass 불가)
...
Does NOT guarantee:
  ...
  - **실명 은닉** (`유명 컨설팅사 2024: 34%`) — 화이트리스트 미등록 일반 표현 (M3, L3 의존)
  - **어순 변경** (`2024년 Harvard 연구에서 34%`) — 기관명이 연도·수치 뒤에 오는 경우 정규식 미매칭 (M3, L3 의존)
  - **익명화** (`A 글로벌 기관 2024년 조사: 34%`) — 가명화된 fabrication (M3, L3 의존)

Failure modes:
  - 정규식 false positive → 예외 조건 추가로 해소 (AC Test Scenarios 확장)
  - 정규식 false negative (scope IN 내) → 화이트리스트 확장 또는 접미어 추가
  - 정규식 false negative (scope OUT) → L3 semantic 검증에 위임, L4 finalize 게이트가 자동 호출 강제
  - 우회 입력 (`McKinsey 2024: 34%` + 가짜 URL) → L3가 WebFetch로 탐지
  - Exception bypass 시도 (`자체 조사:` prefix만 붙임) → 복합 조건으로 차단
  - bypass 성공적 우회 (실명 숨김·어순 변경·익명화) → M3 — L2 miss, L3에서 탐지
...
| # | 입력 문자열 | L2 판정 | 이유 |
|---|---|---|---|
| BP1 | `"자체 조사: McKinsey 2024 리포트 34%"` | reject | `자체 조사` 뒤에 `(n=N)` 또는 `file:` 없음 → self_data 미발동 |
| BP2 | `"McKinsey 2024: 34% (§ 참고)"` | reject | `§` 뒤 숫자 없음 → cross_ref 미발동 |
| BP3 | `"통계청 KOSIS 2024: 34% (Appendix)"` | reject | `Appendix` 뒤 식별자 없음 → cross_ref 미발동 |
...
| CM3 | 한 불릿: `"Harvard 2024 연구에서 34% 성장"` | reject | 같은 문장 내 기관+연도+수치 — 정당한 매칭 |
