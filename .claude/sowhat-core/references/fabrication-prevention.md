...
L1에서 L0 규칙(AI 경로 엄격 + 사용자 경로 중간)을 구체화하는 워크플로우:
...
**산출물 레이어 (외부 공유 — cycle 4에서 L4 게이트 강화):**
- `draft` — 최종 산출물 생성. L1 차단 + L4에서 `unverified` 플래그 1건도 없어야 진행
...
## L0 입력 제약 — 구현 명세 (cycle 4 신설)

L0는 Toulmin 필드 작성 시점에 작동하는 **근본 방어선**. AI 경로와 사용자 경로를 분리 처리하여 할루시네이션 원천 차단과 사용자 작성 편의성을 동시에 달성.
...
Output:         AI 경로:    { allowed: true/false, reason, alternative_forms[] }
...
Postconditions: AI 경로 거부 시: 선택지 메뉴에서 해당 옵션이 제시되지 않거나
                                 플레이스홀더/정성 기술로 대체됨
                사용자 경로 플래그 시: Toulmin 필드에 `unverified: true` 메타 부착됨
                                       (settle은 허용되지만 L4가 추적)

AI 경로 Guarantees (엄격):
  - 다음 카테고리 구체값은 retrieval 통과 없이 **제안 자체가 금지**:
    · 수치 (%, 배, 금액, 건수)
    · 기관명 (영문·한글 화이트리스트 + 접미어 패턴)
    · 인물명 (Smith et al., 김현철 교수 등)
    · 보고서·논문·책명
    · 제품·법안·사건 고유명
    · URL
  - retrieval 경로 3가지만 허용:
    1. `research/#NNN` finding ID (이미 /sowhat:research 통과)
    2. `file:{path}` / `dir:{path}` (사용자 실존 자료)
    3. 대화 세션 내 Sub-Research 실행 결과 (영수증 검증 통과)
  - 위 3가지 없으면 AI는 다음만 제시:
    · 정성 기술 (`"업계 벤치마크 수준의 이탈률"`)
    · 플레이스홀더 (`"{기관} {연도}: {수치}"`)
    · "🔍 Sub-Research 실행" 옵션
...
## L4 게이트키핑 — 구현 명세 (cycle 4: draft 경로 + unverified 차단 추가)

cycle 3까지 L4는 finalize/finalize-planning에서 L3를 자동 실행하는 게이트였다. cycle 4에서 **draft 경로를 추가**하고 **unverified 플래그 집계 차단**을 도입 — sowhat의 핵심 목적인 "settle 후 draft 공유" 경로에서 fabrication 유출을 원천 차단.
...
Preconditions:  finalize-planning: 모든 기획 섹션 settled
                ...
                unverified 플래그 집계 가능 (섹션 파일 메타 스캔)

Postconditions: 통과 시 → 다음 레이어 전환 또는 export/draft 생성
                차단 시 → 즉시 중단, 사용자에게 해소 경로 안내

Guarantees:
  - **unverified 플래그 차단** (cycle 4 신규):
    · finalize-planning / finalize / draft 실행 시 먼저 모든 settled 섹션의 `unverified: true` 메타 집계
    · 1건이라도 존재 시 즉시 중단 + 해소 경로 안내:
      "{N}건의 unverified 항목을 먼저 해소하세요:
         - {섹션}.{필드}: {값}
       해소: /sowhat:research 또는 /sowhat:revise로 출처 추가"
  - finalize-planning: challenge 자동 실행 **생략 불가**
  - finalize: challenge 자동 실행 (기본값)
  - **draft: unverified 차단 + 기존 challenge 자동 실행** (cycle 4 신규)
    이것이 "settle 후 draft 공유" 경로에서 fabrication 유출을 완전 차단한다

Does NOT guarantee:
  - `finalize --force` 시 L3·unverified 차단 모두 건너뜀 (ESH1, 사용자 명시 책임)
  - 사용자가 finalize/draft를 전혀 실행하지 않으면 L4 작동 안 함 (ESH2)
    (단, sowhat의 목적은 "settle 후 draft 공유"이므로 이 시나리오는 주된 사용 패턴 아님)
  - 사용자가 unverified 플래그를 수동 제거한 경우 (spec 위반)

Failure modes:
  - L3 critical 이슈 → 중단 + 리포트
  - L3 타임아웃 → challenge.md 타임아웃 핸들러
  - 네트워크 장애 → L3 partial + 인간 판단
  - unverified 집계 실패 (섹션 파일 메타 파싱 에러) → 보수적으로 차단

Depends on: L3 구현 호출 가능
            finalize-planning / finalize / draft의 게이트 블록
            섹션 파일의 unverified 메타 스키마
```

> **`--force` 주의**: `finalize --force` / `draft --force`는 L3·unverified 차단 모두를 건너뛴다. 이 escape hatch를 사용하면 fabrication 방어선이 전부 무너진다. `--force` 사용 시 명시적 경고 checkpoint가 있다.

### draft 경로 강화의 근거 (cycle 4)

sowhat의 주된 사용 패턴은 **"settle 후 draft로 외부 공유"**. 이전까지 draft는 기획 섹션의 fabrication을 그대로 상속할 수 있었다 (Scenario A 취약). cycle 4에서 draft도 L4 게이트에 포함하여:

- settle 단계에서 unverified 플래그 붙은 항목은 draft에 포함되지 못함
- 사용자는 draft 실행 시 "어떤 섹션의 어떤 인용이 미검증인지" 즉시 알 수 있음
- 해소는 `/sowhat:research`(검증된 finding) 또는 `/sowhat:revise`(출처 추가)

이로써 AI 할루시네이션 원천 차단 (L0 AI-엄격) + 최종 유출 방지 (L4 draft 게이트)가 이중 방어선을 구성.
