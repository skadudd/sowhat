...
**논증 강도 계산** (`references/strength-scoring.md` 참조):
- 각 settled/discussing 섹션의 strength score 계산
- 전체 tree score 계산
- 최약 고리(weakest link) 식별
- 가장 약한 차원(근거/논리/방어/보정) 식별

**주의 필요 항목:**
- `needs-revision` 상태 섹션 → debate 권장
- `invalidated` 상태 섹션이 있고 상위 섹션이 settled이면 → 재검토 필요 알림
- Open Questions 총 건수 > 0
- 논증 강도 60 미만 섹션 → 근거 보강 권장

**7. 논증 부채 집계 (Verification Debt Tracking):**
- 각 섹션(settled 포함)에서 다음 미해결 항목을 수집:
...
- 집계 방법:
  ...
  - `logs/debate/*-round-*.md` 에서 outcome이 `weakened`이고 후속 `strengthen`/`revise` 커밋 없는 섹션 카운트
  - 각 섹션 파일의 Grounds에서 출처명/연도/수치가 없는 항목 카운트

**8. 미처리 노트 집계:**
- `logs/notes.md` 파일이 존재하면:
  - `- [ ]` (미처리) 항목 수 집계
  - 최근 3건의 미처리 노트 내용 추출

**리서치 상태:**
- `research.unreviewed` > 0이면 표시
- Tier 분포 표시 (T1: N, T2: N, T3: N, T4: N)
...
논증 강도 표시 (settled/discussing 섹션이 1개 이상이면):
```

💪 논증 강도: {final_tree_score}/100 [{등급}]
   [{bar}]

   섹션별:
   {section_name}  {score}/100 {emoji}
   {section_name}  {score}/100 {emoji}

   ⚠️ 최약 고리: {weakest_section} ({score}/100) — {약한 이유}
```
...
🔬 리서치
   미검토 파인딩: {N}건
   출처 분포: T1: {N} | T2: {N} | T3: {N} | T4: {N}
```

만료된 파인딩이 있으면 (stale_count > 0):
```

⚠️  만료된 파인딩: {stale_count}건
   영향 받는 섹션: {stale 파인딩의 relevant_sections 중복 제거 목록}
   → /sowhat:research review 로 재검토하세요
```

논증 부채가 있으면 (verification_debt > 0):
```

⚠️  논증 부채 ({total_debt}건)
{open questions > 0:}
   - Open Questions: {N}건 미해결 ({섹션별 목록})
{challenge 미수정:}
   - Challenge 미수정: {N}건 ({section}: {severity} — {이슈 요약})
{출처 미확인:}
   - 출처 미확인: {N}건 ({section}: Grounds[{index}])
{stub 의심:}
   - Stub 의심: {N}건 ({section}: {field})
{debate weakened 미보강:}
   - Debate 미보강: {N}건 ({section}: weakened 후 미수정)
```
...
```
  [1] git add -A && git commit -m "wip: save session state"
  [2] 무시하고 계속
```

**활성 debate 브랜치 있을 때 (브랜치별):**
```
  [1] debate/{branch} merge
  [2] debate/{branch} delete
  [3] 유지
```
...
- 미커밋 변경사항 프롬프트에서:
  ...
  - `[2]`: 미커밋 무시하고 계속 (아무 동작 없음, 다음 입력 대기)

- debate 브랜치 프롬프트에서:
  - `[1]`: debate 브랜치 merge 안내
    ```
  ...
  - `[2]`: debate 브랜치 삭제 확인
    ...
    ⚠️  debate/{branch}를 삭제합니다. 이 작업은 되돌릴 수 없습니다.
      [1] 삭제  [2] 취소
    ```
    [1]: `git branch -d debate/{branch}` 실행
  - `[3]`: 유지 (아무 동작 없음, 다음 입력 대기)

- `[1]`~`[5]`: 해당 커맨드 실행 안내 (Claude Code가 직접 해당 /sowhat: 커맨드를 실행)

- 번호 외 입력: 무시하고 `입력값을 인식할 수 없습니다. 표시된 번호를 입력하세요.`
