...
cycle 7 Plan G parser(`.claude/sowhat-core/bin/source-tag-parser.js`)를 settle 진입 전 **실제로 호출**하여 태그 준수를 구조적으로 보증한다. 이 검증은 LLM semantic 판정이 아닌 코드 기반 정적 검사다.
...
> **출력 규율**: `--json-file`로 호출 — 전체 JSON은 `$LOG`에 영구 기록(dogfooding·cycle 8 audit용)되고 stdout엔 **간결 요약만** 출력된다(대화에 raw JSON 덤프 안 함). parser가 파일 기록을 직접 수행하므로 PowerShell `tee`/`cat` 우회는 불필요.
