...
레퍼런스 분석·캐릭터 프로파일 생성 시 **AI가 생성하는** 콘텐츠에 구체 수치·기관명·연도·인물명·URL 같은 fabrication 가능 고유값을 포함하지 않는다.

- **3차원 분석 결과** (Voice/Flow/Persona): 원본 레퍼런스의 인용만 사용. AI가 "이 캐릭터는 McKinsey 스타일" 같은 외부 기관 비유 금지. 유형 기술(`"간결하고 직설적인 문체"`)만 사용.
- **대표 문단 선정**: 원본 문단을 그대로 인용. AI가 수치를 "평균 14.2자" 형식으로 정량화하지 않음 (이 파일 본문 Step 2 "정량 수치 사용 금지" 규칙과 동일 원칙).
- **캘리브레이션 피드백**: 스타일 차이 기술 시 구체 수치 없이 정성적으로.

구체 내용은 오직 다음 경로에서만 들어온다:
1. 사용자가 제공한 레퍼런스 텍스트 (원본 인용)
2. 사용자 직접 입력 (캘리브레이션 의견)
