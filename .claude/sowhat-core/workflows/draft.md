...
Step 5로 생성된 산출물에 대해 source tag 준수를 검증한다. 단순 LLM-semantic 대조:
...
이 검증은 cycle 1-6의 L1 렌더링 검증 (정규식 + LLM 차집합)을 대체한다. source tag 기반이 더 단순하고 false positive/negative 모두 감소.
