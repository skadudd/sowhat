...
6.5. **L4 Unverified 게이트 (cycle 4 신설 — 생략 불가)**:

   draft는 외부 공유용 산출물 경로다. 기획/명세 섹션의 `unverified` 플래그를 그대로 산출물에 전파하면 fabrication이 외부에 유출된다. `/sowhat:draft --force`가 명시되지 않은 한 반드시 통과해야 한다.

   절차:
   1. 단계 6에서 로드한 모든 섹션 파일의 frontmatter에서 `unverified_items` 배열 집계
   2. 빈 배열이거나 미존재면 → 통과, 단계 7로 진행
   3. 1건이라도 존재하면 → **즉시 중단 + 목록 출력 + 해소 경로 안내**:

      ```
      🔴 Draft 중단 — unverified 항목 {N}건 발견

      기획/명세 섹션에 미검증 인용이 있어 외부 산출물 생성이 차단됩니다.
      fabrication이 draft에 전파되는 것을 막는 L4 게이트입니다.

      [섹션별 unverified 목록]
        02-solution.grounds[2]: "McKinsey 2024: 34%" — en_direct 매칭, 출처 미연결
        02-solution.backing[0]: "통계청 KOSIS 2024" — ko_direct 매칭, 참조 미실존
        03-market.grounds[1]: "유명 컨설팅사 2024: 28%" — 사용자 unverified 플래그
      ...
      해소 경로 (택 1):
        ...
        [4] draft --force                  — 우회 (escape hatch, ESH1 — fabrication 책임은 사용자)
      ```

   4. `--force` 인자가 있으면 경고만 출력 후 진행:

      ```
      ⚠️ --force 사용 — L4 Unverified 게이트 건너뜀

      {N}건의 unverified 인용이 draft 산출물에 포함됩니다. fabrication 가능성 인지
      상태로 진행합니다. 외부 공유 전 수동 검토 필수.
      ```
