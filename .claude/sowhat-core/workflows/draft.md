...
## L1 Fabrication 차단 (필수 — 최종 산출물 경로)

draft는 **외부 공유용 산출물**을 생성한다. AI가 기획 섹션에 **없던** 구체 수치·기관명·연도·인물명·URL을 임의로 추가하는 것을 엄격히 금지한다.

- 기획 섹션의 **기존 인용을 그대로** 옮긴다. 재구성·보강·대체 금지
- 산출물에서 표현을 다듬더라도 **구체값은 원본 그대로**
- 원본에 없는 수치가 산출물에 필요하면 → 사용자 요청 후 `/sowhat:research` 또는 `/sowhat:inject` 경유

draft는 L4 finalize 이후에도 실행 가능하므로, **L3 게이트를 이미 통과한 상태에서도 draft 단계에서 새 fabrication이 투입될 수 있는 유일한 경로**다. 따라서 draft 자체에서 L1 준수가 특히 중요하다.
...
6.5. **L4 Layer 순서 강제 (cycle 5 신설 — AU9 해소, 생략 불가)**:

   draft는 외부 공유용 산출물 경로다. 기획 레이어 확정 전에 draft를 실행하면 challenge가 한 번도 돌지 않은 섹션의 fabrication이 그대로 전파된다.
   ...
   2. `layer == "planning"` AND `/sowhat:finalize-planning` 한 번도 실행 안 됨:
      ...
      이유: challenge Stage 0가 아직 실행되지 않아 scope OUT 카테고리
           (인물명·보고서명·가짜 URL) fabrication이 검증되지 않음.

      다음 중 선택:
        [1] /sowhat:finalize-planning 실행 후 draft 재시도 (권장)
        [2] --force (escape hatch — fabrication 책임 사용자)
      ```

   3. `layer == "spec"` 또는 `"finalized"` → 통과, 단계 6.6으로 진행

6.6. **L4 Challenge Auto-invoke (cycle 5 신설 — AU2 해소, 생략 불가)**:

   기존 spec은 "L4가 L3를 자동 호출"한다고 선언했으나 draft.md에 실제 invocation이 없었다. cycle 5에서 명시적으로 구현.

   절차:
   1. 단계 6에서 로드한 모든 섹션 파일의 frontmatter에서 `last_challenged_at` 타임스탬프 확인
   2. 다음 중 하나라도 해당 → `/sowhat:challenge --all` 자동 실행:
      - `last_challenged_at` 필드 없음 (challenge 한 번도 미실행)
      - `last_challenged_at < updated` (섹션이 challenge 후 수정됨)
   3. 전 섹션 `last_challenged_at >= updated` → 기존 결과 재사용 (효율성)
   4. challenge 실행 중 critical 이슈 발견 → 즉시 중단, 사용자에게 해소 경로 안내
   5. challenge 통과 → 다음 단계 진행

   > **`--force` 사용 시**: challenge 건너뜀. 경고 + ESH1 로그 기록.

6.7. **L4 Unverified 게이트 (cycle 4 신설, 생략 불가)**:

   challenge 통과 후 `unverified_items` 플래그 최종 집계.

   절차:
   1. 단계 6에서 로드한 모든 섹션 파일의 frontmatter에서 `unverified_items` 배열 집계
   2. 빈 배열이거나 미존재면 → 통과, 단계 6.8로 진행
   3. 1건이라도 존재하면 → **즉시 중단 + 목록 출력 + 해소 경로 안내**:

      ```
      🔴 Draft 중단 — unverified 항목 {N}건 발견

      [섹션별 unverified 목록]
        02-solution.grounds[2]: "McKinsey 2024: 34%" — L2-en_direct, 출처 미연결
        02-solution.backing[0]: "통계청 KOSIS 2024" — L2a-finding_miss
        03-market.grounds[1]: "유명 컨설팅사 2024: 28%" — L0-user

      해소 경로 (택 1):
        [1] /sowhat:research {topic}      — 검증된 finding 확보 후 섹션에 매핑
        [2] /sowhat:revise {section}       — URL/file: 경로 수동 추가
        [3] /sowhat:inject {section} {src} — 외부 자료 직접 주입
        [4] draft --force                  — 우회 (escape hatch, ESH1)
      ```

   4. `--force` 인자: 경고 + ESH 로그 + 통과 (명시적 사용자 책임)
   5. 기록: `logs/argument-log.md`에 "L4 draft 게이트 통과/차단" 이벤트 남김

6.8. **L1 렌더링 검증 준비 (cycle 5 신설, cycle 6 snippet 기반 재설계 — AU3/AU15 해소)**:

   이 단계는 Step 5(document generation) 직전에 "planning에 없던 구체값이 산출물에 추가되지 않도록" 준비한다. 실제 검증은 Step 5 직후에 수행.

   **cycle 6 변경 (AU15 해소)**: 정규식 key term 추출은 의미 변환(표현만 바꿈) 감지 불가. snippet 기반 + LLM semantic 대조로 전환.

   절차:
   1. **Snippet 수집 (Step 5 전)**:
      - 각 settled 섹션의 Grounds/Claim/Backing/Warrant의 **불릿/문장 단위 원문**을 모두 수집
      - 수집 대상: 구체값(수치·기관명·인물·보고서·URL·finding ID) 포함된 snippet만 필터링
        (추출 기준: `fabrication-prevention.md` §"금지되는 고유값" 정규식)
      - 변수 `planning_snippets[]`에 저장 — `{section, field, bullet_index, text, concrete_values[]}` 구조

   2. **산출물 생성 후 검증 (Step 5 직후, Step 6 전)**:
      - 산출물 텍스트에서 정규식으로 **구체값 등장 위치 스캔**
      - 각 구체값마다 LLM-semantic 판단:
        > "이 구체값이 `planning_snippets[]` 중 어느 것의 paraphrase 또는 직접 인용인가?
        >   매칭되는 snippet 없음 → 신규 구체값 (fabrication 의심)
        >   매칭되는 snippet 있음 → pass"
      - 매칭 기준: 동일 값 literal 출현 OR 동일 맥락으로 인용 (기관·연도·수치가 원본과 일치)

   3. **차집합 (신규 구체값) 보고**:
      ```
      🔴 L1 렌더링 검증 실패 — 산출물이 기획을 초과합니다

      기획 섹션에 없던 신규 구체값이 산출물에 포함됐습니다:
        - 산출물 위치 {para/line}: "{문제 문장}"
          · 신규 구체값: {value}
          · planning snippets 중 매칭 없음

      이는 AI 렌더링 과정에서 기획을 넘어 구체값을 추가한 경우로,
      fabrication 가능성이 높습니다.

      해소:
        [1] /sowhat:draft 재시도 (다른 스타일/프롬프트)
        [2] 기획 섹션에 해당 구체값 추가 (/sowhat:revise) 후 draft
        [3] --force (우회, 사용자 책임)
      ```

   > **왜 snippet + LLM semantic인가**: cycle 5의 정규식 차집합 방식은 동일 기관명을 표현만 바꿔도 차집합에 나타나 false positive, 반대로 기획의 수치를 산출물에서 다르게 표기하면 false negative. snippet 단위 대조 + LLM semantic 판단이 두 실패 모두 해소.
   > 구현은 draft 실행 주체인 LLM이 이 비교를 직접 수행. AI 생성 도구가 생성 결과를 자기 대조하는 구조.
