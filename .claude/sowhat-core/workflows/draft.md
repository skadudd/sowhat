...
6.8. **L1 렌더링 검증 준비 (cycle 5 신설 — AU3 해소)**:

   이 단계는 Step 5(document generation) 직전에 "planning에 없던 구체값이 산출물에 추가되지 않도록" 준비한다. 실제 검증은 Step 5에서 수행. 여기서는 비교 baseline을 구축.

   절차:
   1. 각 settled 섹션의 Grounds/Claim/Backing 텍스트에서 key terms 추출:
      - 기관명 (en_direct/ko_direct 화이트리스트 + 접미어 패턴 매칭)
      - 수치 (`\d+\.?\d*\s*(조|억|만)?원|\d+\.?\d*%|\$\d+|배|명`)
      - 연도 (`20\d{2}`)
      - 인용 식별자 (`#\d{3}`, `file:`, `§\d+`)
   2. 전체 집합 `T_planning` 변수에 저장
   3. Step 5 document generation 후 산출물 텍스트에서 동일 카테고리 key terms 추출 → `T_rendered`
   4. 차집합 `T_rendered \ T_planning` 계산 — 산출물에만 있는 신규 구체값
   5. 차집합 비어있음 → draft 성공
   6. 차집합 있음 → draft 거부:
      ```
