...
구체값 유입 경로 (오케스트레이터가 `<research_findings>` 태그로 주입):

1. `research/` 디렉터리에 이미 있는 #NNN finding
2. Round 1 자동 스폰된 Research-Agent 결과 (영수증 검증 통과)
...
에이전트 출력의 각 공격 항목·방어 항목은 `[source:...]` 태그를 갖는다:

- 논리 구조 공격/방어: `[source:inference]`
- `<research_findings>` 인용: `[source:#NNN]` 또는 `[source:file:path]`
- 태그 없거나 AI가 임의 부착한 retrieval 태그는 parser가 drop

에이전트 `<principles>` 블록이 이를 상시 준수한다.
