...
> **전체 스키마**: 디렉터리 구조·글로벌 인덱스(`index.json`)·`series.json`·digest/arc/terminology 스키마 전문은 `references/series-data-model.md` 참조. 아래는 워크플로우 실행에 필요한 요약이다.
...
## 서브커맨드: `create {name}` / `promote` — 시리즈 생성·승격

`$ARGUMENTS`가 `create {name}` 또는 `promote`이면 시리즈 생성/승격 분기로 진입한다. 기존 프로젝트 감지·승격 모드, 시리즈 메타 입력(이름·제목·독자·캐릭터·에피소드 기획·arc·thesis), 파일 생성, 글로벌 인덱스 등록, 커밋까지 전체 절차는 **`references/series-create.md`를 읽고 그대로 따른다.**
