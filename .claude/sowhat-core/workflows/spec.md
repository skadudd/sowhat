...
> **AI Content Boundary**: 명세 필드(Actors/Data Model/API/Edge Cases/AC)의 구체 수치·임계값은 기획 Grounds에서 도출(`[source:#NNN]` 보존) 또는 사용자 입력(`[source:user]`)만. 없으면 플레이스홀더(`[source:placeholder]`)로 남기고 핑퐁 중 확정. 상세: `references/ai-content-boundary.md`.
